...
ggplot(dat, aes(x=est_age, color=loneliness_issues)) +
  geom_density()
# Add mean lines
p <-ggplot(dat, aes(x=est_age, color=loneliness_issues)) +
  geom_density() +
  
  labs(title="Age Density by Loneliness Issues",x="Age(yrs)", y = "Density")
p
